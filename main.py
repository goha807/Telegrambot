...
# ================= КОНФІГУРАЦІЯ =================
# Краще використовувати змінні оточення Railway для токена
BOT_TOKEN = os.getenv('BOT_TOKEN', "8213254007:AAFQkGiQqi1YirAvF4VuGcF3CL6WpqFVSGA") 
ADMINS_IDS = [1813590984] # ВАШ ID
MAX_SIZE = 50 * 1024 * 1024  # 50 MB
SPAM_DELAY = 2.0 # Затримка анти-спаму (секунди)
DATA_FILE = 'data/bot_data.json' # Шлях до файлу збереження (для Railway Disk)
...
duel_data = {}
promocodes = {}
required_channels = [] # Тепер список каналів [{'id': ..., 'username': ...}, ...]
...
LANGUAGES = {
    "ua": {
        ...
        "channel_set": "✅ Канал для підписки додано: {username}",
        "channel_removed": "✅ Канал {username} видалено зі списку обов'язкових.",
        "channel_list": "📋 *Список обов'язкових каналів:*\n{channels}",
        ...
        "channel_unset": "✅ Обов'язкову підписку на канали вимкнено.",
    },
    "en": {
...
def save_data():
    ...
    try:
        data_to_save = {
            'user_data': user_data,
            'promocodes': {k: {**v, 'expires': v['expires'].isoformat()} for k, v in promocodes.items()},
            'required_channels': required_channels
        }
        # Конвертуємо datetime об'єкти в строки для JSON
        for uid, stats in data_to_save['user_data'].items():
            if stats.get('vip_expiration'):
                stats['vip_expiration'] = stats['vip_expiration'].isoformat()
            if stats.get('unlimited_dl_expires'):
                stats['unlimited_dl_expires'] = stats['unlimited_dl_expires'].isoformat()
...
def load_data():
    ...
    if os.path.exists(DATA_FILE):
        try:
            ...
            # Відновлення дат у користувачів
        ...
        print("📂 Файл даних не знайдено, починаємо з чистого аркуша.")
...
def handle_shutdown(signum, frame):
    print("🛑 Отримано сигнал зупинки. Зберігаємо дані...")
    save_data()
    sys.exit(0)
...
async def is_user_subscribed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if missing_channels:
        ...
        # Якщо всі канали підписані, видаємо нагороду (якщо ще не видавали)
        stats = get_user_stats(user_id)
        if not stats.get('has_channel_reward', False):
            reward = 100 # Бонус за підписку
            stats['stars'] += reward
            # --- ДОДАЄМО VIP НА 1 ДЕНЬ ---
            ...
            save_data() # Зберегти прогрес нагороди
        return True
...
async def handle_download(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1 = VIP (найвищий)
    # 5 = Priority Pass (середній)
    # 10 = Звичайний (низький)
    priority = 10
    ...
    # Додаємо в PriorityQueue. Елементи сортуються по першому значенню (priority).
    # timestamp додається, щоб зберігати порядок FIFO в межах одного пріоритету.
    ...
    while True:
        try:
            ...
            async with download_in_progress:
                ...
                stats = get_user_stats(user_id)
                # Перевірка коштів (на випадок, якщо зірки зникли поки чекав у черзі)
                ...
                try:
                    ...
                    if size > MAX_SIZE:
                        ...
                        save_data() # Зберегти після успішного завантаження
                except Exception as e:
...
async def download_media(query, audio=True, quality="best"):
    ...
    # --- КОНФІГУРАЦІЯ YT-DLP ДЛЯ MERGING ---
    if audio:
        if quality == "best":
            fmt = "bestaudio/best"
        else:
        ...
        # Для відео ми змушуємо yt-dlp з'єднувати найкраще відео і найкраще аудіо
        if quality == "best":
        ...
        opts = {
            ...
            "noplaylist": True,
            "ignoreerrors": True,
            "merge_output_format": "mp4", # Важливо для TikTok/YouTube
...
async def create_promo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        get_text(context, "promo_created").format(
            code=code, reward=reward, uses=uses, expires=expires.strftime('%Y-%m-%d %H:%M')
        )
    )
    save_data()
...
async def remove_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(required_channels) < initial_len:
        log_action(update.effective_user, f"Видалив канал: {username}")
...
async def list_channels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not required_channels:
        await update.message.reply_text("📋 Обов'язкових каналів немає.")
        return
    response = get_text(context, "channel_list").format(
...
async def unset_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    log_action(update.effective_user, "Вимкнув обов'язкову підписку")
...
async def main():
    global application
    # Завантаження даних перед запуском
    load_data()
